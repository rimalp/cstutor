...
function(data, status){
	...
	for(var i=0; i<response.length; i++){
		...
		//get projects
		getProjectsForCourse(course, getProjectsForCourseCallback(course/*, i==response.length-1*/));
		
		//get students
		getStudentsForCourse(course, getStudentsForCourseCallback(course/*, i==response.length-1*/));
	}
});
...
var loadedGraphs = false;
function displayDetailStudentProject(student, project, back){
	clearDetailView();
	showDetailView();
	
	if(!back)
		backButtonStack.push(getButtonDiv(student.title + "'s " + project.title, function(){backButtonStack.pop(); displayDetailStudentProject(student, project, true); updateGraph(currentGraph);}));
	...
	if(!loadedGraphs){
		getTopLevelGraphsForStudentProject(project.course, project, student,
			function(data, status){
...
function newGraphFunction(){
	...
	var newGraph = (currentGraphHistory.graphs.length > 0) ? currentGraphHistory.graphs[currentGraphHistory.graphs.length-1].clone() : new Graph();
